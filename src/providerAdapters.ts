...
function providerFallbackAuthHeaderName(fields: string[]): "authorization" | "x-api-key" | "api-key" | "" {
...
function switchProviderAuthHeader(headers: Record<string, string>, target: "authorization" | "x-api-key" | "api-key"): void {
...
function providerAuthCredentialValue(headers: Record<string, string>): string {
  for (const name of ["authorization", "x-api-key", "api-key"]) {
    const key = headerKey(headers, name);
...
function openaiChatMessages(request: ModelRequest): Array<Record<string, unknown>> {
  ...
  const images = shouldOmitOpenAIChatImage(request.profile) ? [] : inputImages(request);
  if (!images.length) return openAIChatTextContentFormat(request.profile.bodyExtra?.openAIChatTextContentFormat ?? request.profile.bodyExtra?.openAIChatTextContent) === "string"
...
function openaiResponsesInput(request: ModelRequest, fallbackSystem = ""): OpenAIResponsesInputItem[] {
  ...
  if (!shouldOmitOpenAIResponsesImage(request.profile)) {
...
function shouldOmitOpenAIChatImage(profile: ProviderProfile): boolean {
...
function shouldOmitOpenAIResponsesImage(profile: ProviderProfile): boolean {
...
function shouldOmitGenericImageInput(profile: ProviderProfile): boolean {
...
function openAIChatTextContentFormat(value: unknown): "blocks" | "string" {
  const normalized = String(value || "").trim().toLowerCase().replace(/[-_\s]+/g, "");
...
function providerFieldHintPointerPath(value: string): string {
  const text = String(value || "").trim();
  if (!/^#?\//.test(text)) return "";
  const segments = text
    .replace(/^#/, "")
...
function providerDetailMentionsCanonicalField(detail: string, field: string): boolean {
...
function canonicalProviderFieldHint(value: string): string {
...
function providerFallbackFieldPresent(body: Record<string, unknown>, field: string, protocol = ""): boolean {
...
function providerTextFieldPresent(body: Record<string, unknown>, field: "format" | "verbosity"): boolean {
...
function rejectedAnthropicAuthHeaderField(detail: string): string {
  const text = String(detail || "").toLowerCase();
...
function isProviderHeaderFallbackField(field: string): boolean {
...
function rejectedOpenAIChatImageContentField(body: Record<string, unknown>, detail: string): string {
...
function openAIChatHasImagePart(body: Record<string, unknown>): boolean {
...
function rejectedOpenAIResponsesInputImageField(body: Record<string, unknown>, detail: string): string {
...
function openAIResponsesInputHasImage(body: Record<string, unknown>): boolean {
  return openAIResponsesInputImageParts(body).length > 0;
}

function openAIResponsesInputImageParts(body: Record<string, unknown>): Array<Record<string, unknown>> {
  const input = Array.isArray(body?.input) ? body.input : [];
  return input.flatMap((item: any) => Array.isArray(item?.content) ? item.content : [])
    .filter((part: any) => part?.type === "input_image" && part && typeof part === "object");
}
...
function removeOpenAIResponsesTextField(body: Record<string, unknown>, field: "format" | "verbosity"): void {
  const text = body.text;
  if (!text || typeof text !== "object" || Array.isArray(text)) return;
  const nextText = { ...(text as Record<string, unknown>) };
...
function removeOpenAIChatImageParts(body: Record<string, unknown>): void {
  const messages = Array.isArray(body.messages) ? body.messages : [];
  body.messages = messages.map((message: any) => {
    const content = Array.isArray(message?.content) ? message.content : null;
    if (!content) return message;
    const text = openAIChatContentPartsToString(content.filter((part: any) => part?.type !== "image_url"));
    return {
      ...message,
      content: text || content.filter((part: any) => part?.type !== "image_url")
    };
  });
}

function switchOpenAIChatArrayMessagesToString(body: Record<string, unknown>): void {
  const messages = Array.isArray(body.messages) ? body.messages : [];
  body.messages = messages.map((message: any) => Array.isArray(message?.content)
    ? { ...message, content: openAIChatContentPartsToString(message.content) }
    : message);
}

function openAIChatHasArrayContent(body: Record<string, unknown>): boolean {
  const messages = Array.isArray(body?.messages) ? body.messages : [];
  return messages.some((message: any) => Array.isArray(message?.content));
}

function openAIChatMessageContentString(content: unknown): string {
  if (typeof content === "string") return content;
  if (Array.isArray(content)) return openAIChatContentPartsToString(content);
  return extractMessageContent(content);
}

function openAIChatContentPartsToString(content: unknown[]): string {
  return content
...
function removeOpenAIResponsesInputImages(body: Record<string, unknown>): void {
  const input = Array.isArray(body.input) ? body.input : [];
  body.input = input.map((item: any) => {
    const content = Array.isArray(item?.content) ? item.content : [];
    return {
      ...item,
      content: content.filter((part: any) => part?.type !== "input_image")
    };
  });
}
...
function omitProviderBodyFields(body: Record<string, unknown>, bodyExtra: Record<string, unknown> | undefined, protocol = ""): Record<string, unknown> {
...
function applyNestedProviderBodyOmitField(body: Record<string, unknown>, field: string, protocol = ""): boolean {
  if (field === "instructions") {
...
function appendProviderURLPath(baseURL: string, path: string): string {
  const parts = splitProviderURLSuffix(baseURL);
  return `${parts.path.replace(/\/+$/, "")}/${String(path || "").replace(/^\/+/, "")}${parts.suffix}`;
}

function trimProviderURLPath(baseURL: string): string {
  const parts = splitProviderURLSuffix(baseURL);
  return `${parts.path.replace(/\/+$/, "")}${parts.suffix}`;
}

function providerURLPath(baseURL: unknown): string {
  return splitProviderURLSuffix(baseURL).path;
}

function splitProviderURLSuffix(baseURL: unknown): { path: string; suffix: string } {
