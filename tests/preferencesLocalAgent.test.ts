...
describe("preferences local-agent config helpers", () => {
  ...
  it("renders a provider setup guide with endpoint and live-check commands", () => {
    ...
    expect(guide).toContain(".env.local config doctor: npm run verify:provider:live -- --doctor --include openai --env-file .env.local");
  ...
  it("uses compatible live-check variables for Anthropic-style routers", () => {
    ...
    expect(guide).toContain(".env.local config doctor: npm run verify:provider:live -- --doctor --include anthropic-compatible --env-file .env.local");
