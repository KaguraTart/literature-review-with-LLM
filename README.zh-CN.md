...
- **候选论文审阅队列**：arXiv / Crossref / Semantic Scholar / Unpaywall 的结果会去重、按可解释优先级排序，也可基于 Semantic Scholar references/citations 做可配置的受控扩展，支持保存人工审阅备注、生成审阅状态看板、应用高置信建议、导出 Markdown 审阅报告、写入 JSONL，并且只在人工确认后导入 Zotero。
...
- 候选论文发现已加入可解释排序、重复项协调、可配置的受控引用网络扩展、人工审阅备注、高置信建议应用、审阅状态看板和 Markdown 候选审阅报告，但更完整的全文筛选流程还需要继续加强。
...
- 在当前审阅状态看板和可配置引用网络策略基础上，继续加强候选论文全文筛选、排除理由和证据链记录流程。
