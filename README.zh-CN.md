...
- 已支持单轮图片附件提问和 `图表/截图解析` 技能，并加入可复核校正的本地 OCR 元数据、结构化视觉 OCR / 表格重建输出契约、最近一次图表解析回答的 Markdown 导出报告、从重建 Markdown 表格解析出的 JSON/CSV sidecar、从表格/OCR/文本抽取的低置信图表数据草稿、从模型回答中抽取的像素/坐标数据草稿、显式坐标轴校准锚点导出，以及针对坐标轴校准、置信度、证据标签和点位覆盖的自动质量审阅；图表、表格和手写笔记的理解质量仍取决于模型能力和本地 OCR 语言包，像素坐标和校准锚点都是可复核草稿，不是精确自动数字化结果。
...
- 在当前可复核校正的本地 OCR、提示词级视觉 OCR 契约、JSON/CSV 表格 sidecar 导出、可复核图表数据草稿、模型估计像素/坐标草稿、坐标轴校准锚点导出和自动质量审阅基础上，继续增强更密集点位抽取、更精确自动数字化和校准质量诊断。
