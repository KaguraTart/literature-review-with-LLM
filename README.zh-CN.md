...
- `Model`：使用的模型名称，例如 MiniMax、DeepSeek 或本地模型服务中的模型 id。优先从当前厂商的模型下拉框选择；OpenRouter、LiteLLM、Cline API 这类聚合服务会额外显示 `模型厂商` 下拉，可以先选 Anthropic、Google Gemini、OpenAI、DeepSeek、MiniMax 等厂商，再选具体模型。点击 `加载模型列表` 会先显示内置推荐模型；如果已填写 API key 且接口支持在线模型列表，会继续追加厂商返回的在线模型。Azure OpenAI deployment 和私有网关通常仍需要通过 `自定义/私有部署模型` 改成你自己的 deployment/model 名。
...
- Cline API 使用 `https://api.cline.bot/api/v1` 作为 OpenAI-compatible Chat endpoint。模型下拉会列出常用的 `provider/model` 路由 ID，例如 Anthropic、Gemini、OpenAI、DeepSeek、xAI、MiniMax 等选项；该预设默认声明支持图片输入，但真实图片理解能力仍取决于被路由到的具体模型。
