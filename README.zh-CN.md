...
> 当前状态：已经可以用于单篇论文阅读、摘要、图片提问、图表/截图结构化解析、单篇论文阅读日志导出、多篇论文对比与 Markdown 文献矩阵报告、集合级主题聚类工作区和受控两跳引用网络扩展，但仍处于早期版本。跨集合综述和更强的跨文献分析还在计划中。
...
- **Markdown 原生工作流**：生成本地 Markdown 摘要文件，自动链接回 Zotero；回答可复制为原始 Markdown，也可以导出带证据标签的论文阅读日志，或预览后写回摘要文件。
...
- **候选论文审阅队列**：arXiv / Crossref / Semantic Scholar / Unpaywall 的结果会去重、按可解释优先级排序，也可基于 Semantic Scholar references/citations 做受控两跳扩展，导出 Markdown 审阅报告，写入 JSONL，并且只在人工确认后导入 Zotero。
...
- 设置面板可调整模型、咨询模式、论文元信息、会话、阅读日志导出和候选论文工具。
...
- 多篇论文对比目前限制在工作台上下文内，默认最多纳入 5 篇对比论文，并已支持导出 Markdown 文献矩阵；collection 批量生成已加入启发式主题聚类和集合级 synthesis 矩阵，但聚类是确定性规则，需要在正式写作前人工复核。
- 已支持单轮图片附件提问和 `图表/截图解析` 技能，但图表、表格和手写笔记的理解质量仍取决于模型能力；暂未做本地 OCR 或图表数据重建。
- 公式渲染仍是轻量支持，不是完整 TeX 引擎。
- 论文阅读日志目前是带证据摘录和人工填写字段的结构化 Markdown 草稿，还不是完整的长篇综述报告。
- 候选论文发现已加入可解释排序、重复项协调、受控两跳引用网络扩展和 Markdown 候选审阅报告，但更可配置的深层网络扩展策略和更完整的审阅流程还需要继续加强。
...
- 完善跨集合聚类、更丰富的 synthesis 矩阵和更正式的批量报告生成。
- 增强图片、PDF 图表、表格和实验结果的结构化抽取质量，加入本地 OCR/表格重建。
- 加强可配置的深层引用网络扩展策略和更完整的候选论文审阅流程。
- 补充更多厂商配置截图和教程。
- 增加 Zotero 侧边工作台 UI 测试。
- 在当前候选审阅报告、文献矩阵报告和论文阅读日志之外，增加开题笔记、正式综述报告等导出模板。
