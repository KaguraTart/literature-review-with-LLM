...
- **集合级综述工作区**：对 collection 批量生成时，会写出 `papers.json`、论文笔记索引、方法矩阵、研究空白矩阵、启发式主题聚类、带主张支持分、主张风险和证据轨迹的综合主张矩阵、综合冲突与缺口台账、综合路线图、研究问题卡、研究想法列表、图表复核批量索引、手动综述草稿、带主张证据审计的正式综述报告草稿、联网检索证据包 `literature-search-evidence.*.md`、外部候选文献 JSON，以及最终 `collection-literature-review.*.md`；同时维护全局 `collections/index.json` 和带图式聚类图谱、综合布局板、带排序分的聚类证据卡、主题归并复核板、主题桥接板、带缺口优先级分和排序线索的重复缺口看板、优先级看板和图表复核下钻卡片的跨集合综合地图。
...
- 在当前启发式聚类评分、综合布局分区、主张支持分/风险标签/证据轨迹、证据卡排序分、缺口优先级分、连接证据、复核风险标注和阈值校准板基础上，继续校准综合路线图和最终报告生成。
