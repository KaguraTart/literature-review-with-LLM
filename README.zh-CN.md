...
> 当前状态：已经可以用于单篇论文阅读和摘要，但仍处于早期版本。多篇论文对比、批量综述、截图输入等能力还在计划中。
...
- **多模型厂商配置**：同一个设置页中配置 MiniMax、DeepSeek、OpenAI-compatible、Anthropic、Gemini OpenAI-compatible、OpenRouter、DashScope、SiliconFlow、Ollama、LM Studio 等档案。
...
2. 打开 `Tools -> Markdown 摘要设置`，配置模型厂商、Base URL、API key 和模型名称。
...
- `默认接口档案`：当前启用的模型档案。
- `Provider`：选择厂商或协议预设，例如 MiniMax、DeepSeek、OpenAI-compatible、Anthropic、Gemini OpenAI-compatible、OpenRouter、DashScope、SiliconFlow、Ollama、LM Studio、Local Agents 等。
