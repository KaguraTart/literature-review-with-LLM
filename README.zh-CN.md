...
- `Model`：使用的模型名称，例如 MiniMax、DeepSeek 或本地模型服务中的模型 id。优先从当前厂商的模型下拉框选择，也可以点击 `刷新模型` 先显示内置推荐模型；如果已填写 API key 且接口支持在线模型列表，会继续追加厂商返回的在线模型。内置档案会在模型名稳定的 provider 上预填推荐模型；Azure OpenAI deployment 和私有网关通常仍需要通过 `自定义模型` 改成你自己的 deployment/model 名。
