...
- **集合级综述工作区**：对 collection 批量生成时，会写出 `papers.json`、论文笔记索引、方法矩阵、研究空白矩阵、启发式主题聚类、综合主张矩阵、综合冲突与缺口台账、综合路线图、研究问题卡、研究想法列表、手动综述草稿、正式综述报告草稿，以及全局 `collections/index.json` 和跨集合综合地图。
- **图片提问与图表解析**：工作台支持粘贴截图、拖入图片或选择本地图片；内置 `图表/截图解析` 技能可把论文图片、表格和实验结果整理成带视觉 OCR 文本、表格/数据重建、证据映射和复核清单的结构化 Markdown。
...
- 多篇论文对比目前限制在工作台上下文内，默认最多纳入 5 篇对比论文，并已支持导出 Markdown 文献矩阵；collection 批量生成已加入启发式主题聚类、有证据来源的综合主张矩阵、综合冲突与缺口台账、综合路线图、正式综述报告草稿和跨集合综合地图，但聚类是确定性规则，报告在正式写作前仍需要人工复核。
- 已支持单轮图片附件提问和 `图表/截图解析` 技能，并加入结构化视觉 OCR / 表格重建输出契约，但图表、表格和手写笔记的理解质量仍取决于模型能力；暂未接入本地 OCR 引擎或自动图表数据重建。
- 公式渲染仍是轻量支持，不是完整 TeX 引擎。
- 论文阅读日志和正式综述草稿目前是带证据摘录和人工填写字段的结构化 Markdown 草稿，仍需要人工编辑后才能作为完整长篇综述使用。
- 候选论文发现已加入可解释排序、重复项协调、可配置的受控引用网络扩展、人工审阅备注、结构化全文筛选阶段、排除理由、高置信建议应用、审阅状态看板、证据链复核队列、来源证据摘录、已导入且附加 PDF 候选项的 Zotero 全文索引证据摘录（含命中上下文、匹配到的批注页标签、indexed-text 定位、分页符或 `Page 12` 这类独占页标记存在时的 best-effort 页提示和短哈希）和 Markdown 候选审阅报告，但未批注 PDF 文本的真正页码级来源到 PDF 证据抽取仍需要继续加强。
...
- 完善跨集合聚类，并继续精修全局综合地图、集合级综合主张、综合路线图和报告生成。
- 在当前提示词级视觉 OCR/表格重建契约基础上，继续接入本地 OCR、表格解析和图表数据重建，用于截图、PDF 图表、表格和实验结果。
- 在当前全文筛选阶段、排除理由、审阅状态看板、证据链复核队列、来源证据摘录、带命中上下文、匹配到的批注页标签、best-effort 分页符/页标记提示、定位和短哈希的 Zotero 全文索引证据摘录和可配置引用网络策略基础上，继续补充未批注 PDF 文本的真正页码级来源到 PDF 证据抽取。
