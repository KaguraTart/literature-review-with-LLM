...
- 在当前 Zotero 页文本、本地路径/base64 bridge、带稀疏页选择的受控 OCR fallback、空白或失败 OCR 页的高 DPI、有界 PSM 与灰度预处理自动修复重试、带人工页数上限的显式整篇 OCR opt-in、候选报告抽取质量行、结构化抽取质量诊断、逐页 OCR 置信度/修复信号、OCR 风险摘要、PDF 字节不可访问/bridge 请求失败诊断和 indexed-text fallback 后续动作基础上，继续增强无 bridge 原始字节抽取、灰度渲染之外的 OCR 纠偏/阈值预处理和更强的扫描件版面恢复能力。
