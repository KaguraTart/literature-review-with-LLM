...
> 当前状态：已经可以用于单篇论文阅读、摘要、图片提问、图表/截图结构化解析、单篇论文阅读日志导出、正式综述草稿导出、多篇论文对比与 Markdown 文献矩阵报告、带证据支持综合主张和正式综述报告草稿的集合级主题聚类工作区，以及可配置的受控引用网络扩展，但仍处于早期版本。跨集合综述和更强的跨文献分析仍在继续完善。
...
- **Markdown 原生工作流**：生成本地 Markdown 摘要文件，自动链接回 Zotero；回答可复制为原始 Markdown，也可以导出带证据标签的论文阅读日志和正式综述草稿，或预览后写回摘要文件。
...
- **集合级综述工作区**：对 collection 批量生成时，会写出 `papers.json`、论文笔记索引、方法矩阵、研究空白矩阵、启发式主题聚类、综合主张矩阵、研究问题卡、研究想法列表、手动综述草稿和正式综述报告草稿。
...
- **候选论文审阅队列**：arXiv / Crossref / Semantic Scholar / Unpaywall 的结果会去重、按可解释优先级排序，也可基于 Semantic Scholar references/citations 做可配置的受控扩展，导出 Markdown 审阅报告，写入 JSONL，并且只在人工确认后导入 Zotero。
...
live 检查也支持请求体覆盖。`--body-extra-json` 会作用于本次选择的所有 case；也可以使用更细的环境变量，例如 `OPENAI_COMPATIBLE_BODY_EXTRA_JSON`、`OPENAI_RESPONSES_COMPATIBLE_BODY_EXTRA_JSON`、`ANTHROPIC_COMPATIBLE_BODY_EXTRA_JSON`、`OPENAI_BODY_EXTRA_JSON`、`ANTHROPIC_BODY_EXTRA_JSON`。
...
- 多篇论文对比目前限制在工作台上下文内，默认最多纳入 5 篇对比论文，并已支持导出 Markdown 文献矩阵；collection 批量生成已加入启发式主题聚类、有证据来源的综合主张矩阵和正式综述报告草稿，但聚类是确定性规则，报告在正式写作前仍需要人工复核。
...
- 候选论文发现已加入可解释排序、重复项协调、可配置的受控引用网络扩展和 Markdown 候选审阅报告，但更完整的审阅流程还需要继续加强。
...
- 完善跨集合聚类，并继续精修集合级综合主张和报告生成。
- 增强图片、PDF 图表、表格和实验结果的结构化抽取质量，加入本地 OCR/表格重建。
- 在当前可配置引用网络策略基础上，继续加强更完整的候选论文审阅流程。
- 补充更多厂商配置截图和教程。
- 在当前候选审阅报告、文献矩阵报告、论文阅读日志和正式综述草稿之外，增加开题笔记、课题申报笔记和更精修的期刊/报告格式模板。
