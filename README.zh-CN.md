...
- **候选论文审阅队列**：arXiv / Crossref / Semantic Scholar / Unpaywall 的结果会去重、按可解释优先级排序，也可基于 Semantic Scholar references/citations 做一跳扩展，写入 JSONL，并且只在人工确认后导入 Zotero。
...
- 候选论文发现已加入可解释排序、重复项协调和一跳引用网络扩展，但多跳网络扩展和更完整的审阅流程还需要继续加强。
