...
> 当前状态：已经可以用于单篇论文阅读、摘要、图片提问、图表/截图结构化解析、多篇论文对比与 Markdown 文献矩阵报告、集合级主题聚类工作区和一跳引用网络扩展，但仍处于早期版本。跨集合综述和更强的跨文献分析还在计划中。
...
- **候选论文审阅队列**：arXiv / Crossref / Semantic Scholar / Unpaywall 的结果会去重、按可解释优先级排序，也可基于 Semantic Scholar references/citations 做一跳扩展，导出 Markdown 审阅报告，写入 JSONL，并且只在人工确认后导入 Zotero。
...
- 候选论文发现已加入可解释排序、重复项协调、一跳引用网络扩展和 Markdown 候选审阅报告，但多跳网络扩展和更完整的审阅流程还需要继续加强。
