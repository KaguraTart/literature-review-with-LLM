...
- `Model`：使用的模型名称，例如 MiniMax、DeepSeek 或本地模型服务中的模型 id。可以直接从模型下拉框选择内置推荐模型，也可以点击 `加载模型` 获取当前厂商返回的在线模型列表。内置档案会在模型名稳定的 provider 上预填推荐模型；Azure OpenAI deployment 和私有网关通常仍需要改成你自己的 deployment/model 名。
