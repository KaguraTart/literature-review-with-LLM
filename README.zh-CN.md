...
> 当前状态：已经可以用于单篇论文阅读、摘要、图片提问、图表/截图结构化解析、单篇论文阅读日志导出、正式综述草稿导出、多篇论文对比与 Markdown 文献矩阵报告、带正式综述报告草稿的集合级主题聚类工作区，以及可配置的受控引用网络扩展，但仍处于早期版本。跨集合综述和更强的跨文献分析还在计划中。
...
- **集合级综述工作区**：对 collection 批量生成时，会写出 `papers.json`、论文笔记索引、方法矩阵、研究空白矩阵、启发式主题聚类、研究问题卡、研究想法列表、手动综述草稿和正式综述报告草稿。
...
- 多篇论文对比目前限制在工作台上下文内，默认最多纳入 5 篇对比论文，并已支持导出 Markdown 文献矩阵；collection 批量生成已加入启发式主题聚类、集合级 synthesis 矩阵和正式综述报告草稿，但聚类是确定性规则，报告在正式写作前仍需要人工复核。
...
- 完善跨集合聚类、更丰富的 synthesis 矩阵和更精修的集合报告生成。
