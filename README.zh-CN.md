...
- 在当前 Zotero 页文本、本地路径/base64 bridge、受控 OCR fallback、候选报告抽取质量行、结构化抽取质量诊断和逐页 OCR 置信度信号基础上，继续增强整篇扫描件 OCR、PDF 字节不可访问场景、更准确的 OCR 置信度校准和无 bridge 原始字节抽取。
