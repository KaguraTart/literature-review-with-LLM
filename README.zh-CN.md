...
- 在当前 Zotero 页文本、本地 `pdftotext` 和受控 OCR fallback 基础上，继续增强整篇扫描件 OCR、缺少本地文件路径、OCR 质量复核和无 bridge 原始字节抽取。
