...
var ZoteroMarkdownSummaryWorkbench = {
  ...
  applyLanguage() {
    ...
    setButtonLabel("zms-new-conversation", this.t("newConversation"), this.t("newConversation"));
    setButtonLabel("zms-compact-context", this.t("compactContext"), this.t("compactContext"));
    setButtonLabel("zms-copy-session", this.t("copySession"), this.t("copySession"));
  ...
  profileFromSettingsPanel() {
    ...
    const next = hydrateProfile({
      ...
      bodyExtra: workbenchBodyExtraWithModelPickerFeatureHints(this.state.profile.bodyExtra || {}, "zms-profile-model-select", "zms-profile-model"),
      capabilities: {
...
function providerDiagnosticsLabels(outputLanguage) {
  ...
  return {
    ...
    checkModel: "Confirm the model name exists. Prefer the concrete model dropdown, then refresh online models when available.",
...
function workbenchBodyExtraWithModelPickerFeatureHints(bodyExtra, selectId, inputId) {
  const next = { ...(bodyExtra || {}) };
  const picked = workbenchModelPickerFeatureHints(selectId, inputId);
...
function workbenchModelPickerFeatureHints(selectId, inputId) {
  const model = workbenchModelValueFromPicker(selectId, inputId);
  const option = workbenchModelPickerSelectedOption(selectId, model);
...
function workbenchModelPickerSelectedOption(selectId, model) {
  const select = document.getElementById(selectId);
...
function flattenedWorkbenchOptionChildren(element) {
  ...
  for (const child of Array.from(element?.children || [])) {
    if (String(child?.localName || "").toLowerCase() === "optgroup" || Array.isArray(child?.children)) {
      result.push(...flattenedWorkbenchOptionChildren(child));
    }
...
function builtInSkillTemplate(skillId, outputLanguage) {
  ...
  if (skillId === "collection-literature-review") return collectionLiteratureReviewTemplate(common, outputLanguage);
...
function literatureReviewSynthesisTemplate(common, outputLanguage) {
  if (outputLanguage === "zh-CN") {
    return `${common}\n\n生成可直接用于文献综述写作的跨论文综合。若上下文包含 Comparison papers，请把焦点论文和所有对比论文一起综合；若只有当前论文，先输出单篇综述骨架并明确缺少对比论文。不要按论文逐篇流水账总结；请使用 synthesis matrix 思维，把论文按主题、研究问题、方法族、应用场景、证据强弱、分歧和研究空白组织。小方向不一样但问题相近的论文，请放在一个更大的分析框架下讨论；完全不相关的论文，请拆成独立小节或独立分点，不要强行合并。请解释论文关系和研究脉络：继承了什么共同问题，扩展了哪些方法或场景，挑战了哪些假设，留下了哪些未解决问题。使用 Markdown，固定包含：\n\n## 综述主题与范围\n## 综合分析框架\n## 论文分组与研究谱系\n## 方法、数据与实验对比\n## 共同问题、核心共识与关键分歧\n## 批判性评论与证据强弱\n## 研究空白与未解决问题\n## 可写入正文的综述段落草稿\n## 后续补充文献与验证清单\n\n每个判断都必须引用 [metadata]、[chunk:<id>]、[paper2:<id>] 等证据标签；不要把低置信推断写成确定结论。`;
  }
  if (outputLanguage === "ja-JP") {
    return `${common}\n\n文献レビュー執筆に使える横断的な統合を作成してください。Comparison papers がある場合は焦点論文と比較論文をまとめて扱い、ない場合は単一論文のレビュー骨子として不足を明記してください。論文を 1 本ずつ並べるのではなく、テーマ、問題、手法ファミリー、シナリオ、証拠強度、相違点、研究ギャップで統合してください。近いが小方向が異なる論文は大きな枠組みの下にまとめ、完全に無関係な論文は独立したレビュー節として分けてください。章はレビュー範囲、統合フレーム、論文群の分類と系譜、共通課題、方法・データ・実験比較、相違点と証拠強度、批判的コメント、研究ギャップ、本文に使える段落案、追加確認リストを含めてください。各判断には [metadata]、[chunk:<id>]、[paper2:<id>] などの根拠ラベルを付けてください。`;
  }
  return `${common}\n\nCreate a cross-paper synthesis for literature-review writing. If the context contains Comparison papers, synthesize the focal paper together with every comparison paper; otherwise produce a single-paper review scaffold and state that comparison papers are missing. Do not summarize papers one by one. Use synthesis matrix thinking: group by theme, research problem, method family, scenario, evidence strength, disagreement, and research gap. Group related but smaller different directions under a larger framework; split completely unrelated papers into independent sections instead of forcing a merger. Explain research lineage and relationships: what each group inherits, extends, challenges, or leaves unresolved. Use Markdown with these sections:\n\n## Review Scope\n## Synthesis Framework\n## Paper Groups And Research Lineage\n## Shared Problem, Core Consensus, And Key Disagreements\n## Method, Data, And Experiment Comparison\n## Critical Review And Evidence Strength\n## Research Gaps And Open Questions\n## Draft Review Paragraphs\n## Follow-up Literature And Verification Checklist\n\nEvery judgment must cite evidence labels such as [metadata], [chunk:<id>], or [paper2:<id>]. Mark low-confidence inferences explicitly instead of presenting them as settled claims.`;
}

function collectionLiteratureReviewTemplate(common, outputLanguage) {
  const extra = [
...
function assertModelInputSupport(profile, requestInput = {}) {
  const model = String(profile?.model || "").trim();
  if (!model) return;
  if (requestInputImages(requestInput).length && modelLikelyTextOnlyForRequest(profile, "image")) {
    throw new Error(`Selected model ${model} is text-only; choose an image-capable model before sending images.`);
  }
  if (requestInput?.type === "pdf_base64" && requestInput.base64 && modelLikelyTextOnlyForRequest(profile, "pdf")) {
    throw new Error(`Selected model ${model} is text-only; choose a PDF-capable model or switch to extracted text input.`);
  }
}

function modelLikelyTextOnlyForRequest(profile, inputFeature = "") {
  const hints = modelFeatureHintsForWorkbenchProfile(profile);
...
function modelFeatureHintsForWorkbenchProfile(profile) {
...
function modelCapabilityStatusLines(profile, translate = (key) => key) {
  const t = typeof translate === "function" ? translate : (key) => key;
  const textOnly = modelLikelyTextOnlyForRequest(profile);
  const imageMismatch = canUseImageInput(profile) && modelLikelyTextOnlyForRequest(profile, "image");
  const pdfMismatch = canUsePdfBase64Input(profile) && modelLikelyTextOnlyForRequest(profile, "pdf");
...
function openaiChatMessages(messages, requestInput = {}, profile = null) {
  ...
  if (!images.length) {
    const withContext = contextText ? messagesWithAppendedOpenAIChatText(mapped, contextText) : mapped;
    return openAIChatTextContentFormat(profile?.bodyExtra?.openAIChatTextContentFormat ?? profile?.bodyExtra?.openAIChatTextContent) === "string"
      ? withContext.map((message) => ({ ...message, content: openAIChatMessageContentString(message.content) }))
      : withContext;
  }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (error.retryableProviderError && attempt < 3) {
          await delay(providerRetryDelayMs(error, attempt));
          continue;
        }
        throw error;
      }
      ...
      lastError = err;
      if (attempt < 3) {
        await delay(providerRetryDelayMs(err, attempt));
        continue;
      }
    }
  }
...
function providerTokenLimitFallbackFields(body, detail) {
  const fields = [];
  if (
...
function providerDetailSuggestsTokenLimitField(detail, field) {
