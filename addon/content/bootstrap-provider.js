...
function collectProviderErrorDetails(value, details, depth) {
  ...
  const message = firstString(value.msg, value.message, value.detail, value.reason, value.description, value.type, value.code);
...
function moveOpenAIChatSystemIntoMessages(body) {
  ...
  const systemText = messages
    ...
    .map((message) => extractOpenAIMessageContent(message.content))
    .filter(Boolean)
    .join("\n\n");
