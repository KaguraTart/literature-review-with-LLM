...
- Harden PDF page extraction beyond the current Zotero page text, local path/base64 bridge, bounded OCR fallback path, candidate-report extraction-quality rows, and structured extraction diagnostics, especially full-document scanned OCR, attachments with unavailable bytes, richer per-page OCR confidence signals, and no-bridge raw-byte extraction.
