...
- **Provider diagnostics**: OpenAI-compatible, OpenAI Responses, Anthropic, and wrapped router responses are normalized for text, stream errors, model lists, and token usage metadata in saved sessions; the workbench can export a redacted provider diagnostics Markdown report for troubleshooting Base URL, auth, model-list, and live-check settings.
