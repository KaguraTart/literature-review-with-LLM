...
- Harden PDF page extraction beyond the current Zotero page text, local path/base64 bridge, bounded OCR fallback path with sparse-page selection, explicit full-document OCR opt-in under reviewed page caps, candidate-report extraction-quality rows, structured extraction diagnostics, per-page OCR confidence signals, calibrated OCR confidence risk summaries, unavailable-byte/bridge-request diagnostics, and indexed-text fallback follow-up actions, especially no-bridge raw-byte extraction, automatic repair after OCR failures, and stronger scanned-document layout recovery.
