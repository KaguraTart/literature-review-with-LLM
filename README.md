...
- Continue hardening chart digitization beyond the current prompt-level visual OCR contract, editable local OCR metadata, JSON/CSV table sidecar export, reviewable chart-data drafts, dense point-data draft parsing, model-estimated pixel/coordinate drafts, axis-calibration anchor export, linear axis-value inference, calibration-quality diagnostics, automatic quality review, persisted chart review action queues, and in-workbench chart-review state editing; the next gaps are robust non-linear/segmented digitization and richer batch review workflows.
