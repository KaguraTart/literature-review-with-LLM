...
- `Model`: model name used by the selected provider. Pick from the provider-specific model dropdown first, or click `Load model list` to show built-in recommendations and then append the provider's online model list when the profile supports it and an API key is available. Built-in profiles prefill a practical recommended model where the provider has a stable public model name; Azure OpenAI deployments and private gateways may still require your own deployment/model name through `Custom model`.
