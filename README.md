...
- Harden PDF page extraction beyond the current Zotero page text, local path/base64 bridge, bounded OCR fallback path, candidate-report extraction-quality rows, structured extraction diagnostics, and per-page OCR confidence signals, especially full-document scanned OCR, attachments with unavailable bytes, calibrated OCR confidence, and no-bridge raw-byte extraction.
