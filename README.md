...
> Current status: usable but still early. The core single-paper workflow, image-question flow, optional local OCR metadata for images, figure/table extraction prompt, reviewable chart-data drafts from reconstructed tables/OCR, model-estimated pixel/coordinate data drafts, in-workbench chart-review state editing, single-paper reading-log export, proposal-note export with domain writing structures, discipline-style writing examples, writing style templates, field-specific reviewer checklists, paragraph-level revision examples, evidence requirements, and checklists, journal/report-outline export with domain writing structures, venue-specific writing patterns, venue-specific reviewer criteria, venue-specific acceptance examples, discipline-style writing examples, writing style templates, field-specific reviewer checklists, paragraph-level revision examples, longer manuscript paragraph examples, full-section manuscript drafts, evidence requirements, and checklists, provider-specific visual setup examples, formal review-draft export, multi-paper comparison with a reusable Markdown literature-matrix report, evidence coverage maps, synthesis-claim ledgers, pairwise contrasts, gap ledgers, collection topic-cluster workspace with evidence-backed synthesis claims, a conflict/gap ledger, a synthesis roadmap, formal-report writing-readiness gates, section-readiness matrices, final-report calibration matrices, online search evidence, and final collection-level review Markdown, a cross-collection synthesis index/map with graph-style cluster maps, cluster evidence cards, theme-merge review, theme-bridge, recurring-gap, priority boards, and chart-review triage with drilldown details, and configurable bounded citation-network expansion are in place; richer cross-paper analysis is still being improved.
...
- **Markdown-native reading notes**: generate summaries as local Markdown files, link them back to Zotero, copy raw Markdown answers, export evidence-labeled paper reading logs, proposal notes and journal/report outlines with prompt-pack-specific writing structures, discipline-style writing examples, writing style templates, field-specific reviewer checklists, paragraph-level revision examples, venue-specific writing patterns, venue-specific reviewer criteria, venue-specific acceptance examples, longer manuscript paragraph examples, full-section manuscript drafts, evidence requirements, and checklists, plus formal review drafts, and write selected answers back with a preview step.
- **Provider-flexible setup**: use MiniMax, DeepSeek, OpenAI-compatible Chat providers, OpenAI Responses-compatible providers, Anthropic / Anthropic-compatible providers, Gemini OpenAI-compatible endpoints, Azure OpenAI, Vercel AI Gateway, Cline API, Cloudflare AI, GitHub Models, Hugging Face, DeepInfra, Fireworks AI, Cerebras, NVIDIA NIM, SambaNova, OpenRouter, DashScope, SiliconFlow, Ollama, LM Studio, and other profiles from one settings page with an in-app setup guide, provider-specific visual setup examples, and live-check command template.
...
- Export proposal notes and journal/report outlines with evidence labels, writing-positioning fields, prompt-pack-specific domain writing structures, discipline-style writing examples, writing style templates, field-specific reviewer checklists, paragraph-level revision examples, venue-specific writing patterns, venue-specific reviewer criteria, venue-specific acceptance examples, longer manuscript paragraph examples, full-section manuscript drafts, evidence requirements, and manual follow-up checklists.
...
- Generate collection workspace artifacts during collection batch runs, including method matrices, topic clusters, synthesis-claims matrices, synthesis-conflict ledgers, synthesis roadmaps with readiness boards, final-report calibration matrices, final-report action plans with required materials, formal-report writing-readiness gates, section-readiness matrices, section action plans, gap matrices, chart-review batch indexes, review-draft scaffolds, online search evidence, final collection literature-review Markdown, a cross-collection cluster map, ranked cluster evidence cards, theme-merge review board, theme-bridge board, recurring-gap board, priority board, cross-collection chart-review triage with drilldown cards, and a formal review report scaffold.
...
The generated Markdown files are saved under the configured output directory. Use `Choose Folder...` in the Zotero settings page, or the same output-directory field in the workbench settings drawer, to choose a folder with the system file manager. Automatic update sync is enabled by default and can be disabled from either the Zotero settings page or the workbench settings drawer. By default, the plugin creates per-item summary/session files and links summary files back to Zotero.
...
For provider-specific visual cards and short recipes, see [Provider setup examples](docs/provider-setup-examples.md).
...
- The same bridge exposes `extract_pdf_pages` for best-effort page-level text extraction from local PDF files or in-memory PDF bytes. The default text command is `/opt/homebrew/bin/pdftotext`; override it with `LOCAL_AGENT_PDFTOTEXT_BIN=/path/to/pdftotext` before starting the service. When the extracted text is too sparse, callers can enable OCR fallback: the bridge renders a bounded number of pages with `/opt/homebrew/bin/pdftoppm` and OCRs them with Tesseract. The `ocrPageStrategy: "sparse"` mode uses `pdftotext` page breaks to OCR empty or low-text pages across the document, then merges recovered OCR pages with normal text pages. Empty or failed OCR pages are retried once with a higher-DPI render by default; if that retry still returns no text, the bridge tries bounded Tesseract page-segmentation-mode retries from `ocrRepairPsms` (default `6,11`), then preprocessed renders from `ocrPreprocessModes` (default `grayscale,monochrome,adaptive-threshold`) when `ocrPreprocessRepair` is enabled. The adaptive-threshold mode renders a grayscale PGM page and converts it to a local-threshold PBM before OCR, without requiring ImageMagick or another image-processing dependency. Set `ocrAutoRepair: false` to keep the raw first-pass OCR diagnostics. For scanned PDFs that need a broader pass, callers can explicitly set `fullDocumentOcr: true` with `ocrPageStrategy: "all"` and a reviewed `maxOcrPages`; this lifts the normal 12-page OCR cap under a hard safety cap and records `ocr_full_document_used` / `ocr_page_limit_reached` quality warnings when applicable. Override the renderer with `LOCAL_AGENT_PDFTOPPM_BIN=/path/to/pdftoppm`. The returned JSON includes a `quality` object with text length, readable-page counts, empty-page counts, OCR fallback state, full-document OCR scope, OCR repair, PSM-repair, and preprocessing-repair counts, per-page OCR signals (`textChars`, `ocrConfidence`, repair strategy/status, repair PSM, repair preprocessing, empty/error warnings), and warnings such as `ocr_fallback_used`, `ocr_auto_repair_used`, `ocr_psm_repair_used`, `ocr_preprocess_repair_used`, or `sparse_text`.
...
Automatic update sync is enabled by default. Users can turn off `Automatically sync updates` in the Zotero `Literature Review with LLM` settings pane or the workbench settings drawer; when disabled, the plugin stops proactive update prompts and tries to disable this add-on's background update policy. Zotero still controls the global extension update policy.

## Current Limitations

- Multi-paper comparison is available as a workbench workflow, with up to 5 comparison papers by default and a reusable Markdown literature-matrix export that includes evidence coverage, a synthesis-claim ledger, pairwise contrast, and gap-ledger sections. Collection batch runs now generate heuristic topic clusters, evidence-backed synthesis-claims matrices with claim support scores, claim-risk labels, and evidence traces, synthesis-conflict ledgers, synthesis roadmaps with readiness scores, blocking issues, next actions, final-report calibration matrices for report placement and citation strategy, final-report action plans with required materials, a formal review report scaffold with a claim evidence audit, roadmap-readiness board, section-readiness matrix, section action plan, and final-report calibration/action matrices, and a cross-collection synthesis map with graph-style cluster maps, a synthesis layout board, ranked cluster evidence cards, cluster scores, evidence-card rank signals, link-evidence signals, review-risk labels, a cluster-threshold calibration board, theme-merge review, theme-bridge, recurring-gap and priority boards with gap-priority scores and signals, and chart-review triage with source drilldown, but the clustering, layout lanes, claim-support scores, roadmap-readiness scores, section-readiness scores, final-report placement/citation recommendations, required-material suggestions, threshold recommendations, gap-priority weights, ranking weights, and matrix synthesis are deterministic and still need human review before final writing.
- Single-turn image attachments and the `Figure/Table Extractor` skill are supported, including optional reviewable local OCR metadata, a structured visual OCR / table-reconstruction output contract, a reusable Markdown export, JSON/CSV sidecars parsed from reconstructed Markdown tables, low-confidence chart-data drafts extracted from tables/OCR/text, dense point-data drafts recognized from dedicated chart tables, pixel/coordinate data drafts parsed from model answers, explicit axis-calibration anchor export, linear/log/segmented axis-value inference for pixel points when enough anchors are available, range-row calibration for broken-axis segments, a reviewable broken-axis calibration map with pixel/value gaps between segments, segment-level calibration confidence/risk labels, gap-status/risk diagnostics, multi-panel panel-coverage diagnostics, automatic panel split candidate boxes with validation status, validation score, explicit gutter-aware box geometry, and geometry metadata, axis-segment layout diagnostics, axis-break visual-cue detection and review tasks, dense-point confidence checks, an automatic chart-data quality review for axis calibration, calibration-anchor quality, broken-axis segment coverage, panel split validation, confidence, evidence labels, and point coverage, plus a chart batch review board, collection-level chart-review batch index, cross-collection chart-review triage with expandable drilldown cards, machine-readable batch status writeback targets from drilldown, a chart review action queue with default `todo` states, reviewer/due/notes editing inside the workbench, done criteria for human follow-up, and persisted review-state carryover when exporting again. Chart, table, and handwritten-note understanding still depends on the selected model and local OCR language data; dense point lists, panel split boxes, explicit gutter hints, broken-axis visual cues, broken-axis calibration maps, segment confidence/risk labels, gap diagnostics, pixel coordinates, calibration anchors, and inferred axis values are reviewable estimates rather than precise automatic digitization.
- Formula rendering is lightweight. It supports common inline/display math patterns, but it is not a full TeX engine.
- Paper reading logs and formal review drafts are structured Markdown scaffolds with evidence excerpts, formal-report writing-readiness gates, section-readiness matrices, and manual fields. Proposal notes and journal/report outlines now include prompt-pack-specific discipline-style writing examples, writing style templates, field-specific reviewer checklists, and paragraph-level revision examples, and journal/report outlines include venue-specific patterns, venue-specific reviewer criteria, venue-specific acceptance examples, longer manuscript paragraph examples, and full-section manuscript drafts for journal articles, conference papers, review articles, technical reports, and policy/management briefs, but they still need human editing before becoming polished long-form manuscripts.
- Candidate-paper search now has explainable ranking, duplicate reconciliation, configurable bounded citation-network expansion, saved manual review notes, structured full-text screening stages, exclusion reasons, high-confidence recommendation application, a screening board, an evidence-chain follow-up queue, source-evidence snippets, Zotero indexed full-text evidence snippets with hit context, matched annotation page labels, page-level PDF text locators from existing page text, local file paths, or in-memory PDF bytes through the local bridge, a no-bridge raw-byte text fallback for reviewable uncompressed PDF strings and FlateDecode streams, bounded OCR fallback for scanned PDFs with sparse-page selection, automatic higher-DPI, bounded PSM, grayscale/monochrome preprocessing, and local adaptive-threshold preprocessing repair retries for empty or failed OCR pages, explicit full-document OCR opt-in for scanned PDFs under a reviewed page cap, structured extraction-quality diagnostics, per-page OCR confidence and repair signals, calibrated OCR confidence summaries and risk labels, unavailable-PDF-byte, bridge-request, raw-byte, FlateDecode-stream, and indexed-text fallback diagnostics, repeated page-header/footer cleanup, simple line-break dehyphenation, table-of-contents/reference noise downranking, best-effort page hints when indexed text preserves form-feed page breaks or standalone page markers such as `Page 12`, and short hashes for imported candidates with attached PDFs, plus a Markdown candidate-review report. Bridge-based page extraction still depends on accessible PDF bytes and local Poppler/Tesseract tools; raw-byte fallback remains low-confidence for encoded fonts, non-Flate filters, complex object streams, and heavily compressed layouts, while OCR deskew and stronger scanned-document layout recovery still need more work.
...
- Continue calibrating synthesis roadmaps and final report generation beyond the current heuristic cluster scores, synthesis layout lanes, claim support scores/risk labels/evidence traces, roadmap-readiness scores/actions, final-report placement/citation calibration matrix, final-report action plan, formal-report writing-readiness gates, section-readiness matrices, section action plans, evidence-card rank signals, gap-priority scores/signals, link-evidence signals, review-risk labels, and threshold calibration board; the next gaps are paragraph-level ordering, stronger citation-placement calibration, and better separation of mature claims from evidence-building tasks.
- Continue hardening chart digitization beyond the current prompt-level visual OCR contract, editable local OCR metadata, JSON/CSV table sidecar export, reviewable chart-data drafts, dense point-data draft parsing, model-estimated pixel/coordinate drafts, axis-calibration anchor export, linear/log/segmented axis-value inference, range-row broken-axis calibration, reviewable broken-axis calibration maps with segment confidence/risk and gap-status/risk diagnostics, multi-panel layout diagnostics, automatic panel split candidate boxes with validation scoring, explicit gutter-aware geometry scoring, axis-segment coverage diagnostics, axis-break visual-cue review tasks, dense-confidence checks, automatic quality review, batch review boards, collection-level cross-report batch indexes, cross-collection chart-review triage, drilldown cards, batch status writeback targets, persisted chart review action queues, and in-workbench chart-review state editing; the next gaps are automatically detecting gutters from image pixels, pixel-validated broken-axis calibration, and higher-confidence dense chart extraction.
- Harden PDF page extraction beyond the current Zotero page text, local path/base64 bridge, no-bridge raw-byte text fallback for reviewable uncompressed PDF strings and FlateDecode streams, bounded OCR fallback path with sparse-page selection, automatic high-DPI, bounded PSM, grayscale/monochrome/adaptive-threshold preprocessing repair retries for empty or failed OCR pages, explicit full-document OCR opt-in under reviewed page caps, candidate-report extraction-quality rows, structured extraction diagnostics, per-page OCR confidence/repair signals, calibrated OCR confidence risk summaries, unavailable-byte/bridge-request/raw-byte/FlateDecode-stream diagnostics, and indexed-text fallback follow-up actions, especially broader raw-byte extraction for encoded fonts, non-Flate filters, complex object streams, OCR deskew, and stronger scanned-document layout recovery.
