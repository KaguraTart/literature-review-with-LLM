...
- Generate collection workspace artifacts during collection batch runs, including method matrices, topic clusters, synthesis-claims matrices, synthesis-conflict ledgers, synthesis roadmaps, gap matrices, review-draft scaffolds, online search evidence, final collection literature-review Markdown, a cross-collection cluster map, cluster evidence cards, theme-merge review board, theme-bridge board, recurring-gap board, priority board, and a formal review report scaffold.
...
- Continue calibrating cross-collection clustering thresholds, cluster evidence-card ranking, global synthesis-map layout, recurring-gap and priority-board prioritization, collection-level synthesis claims, synthesis roadmaps, and final report generation beyond the current heuristic cluster scores, link-evidence signals, and review-risk labels.
- Continue hardening chart digitization beyond the current prompt-level visual OCR contract, editable local OCR metadata, JSON/CSV table sidecar export, reviewable chart-data drafts, dense point-data draft parsing, model-estimated pixel/coordinate drafts, axis-calibration anchor export, linear/log/segmented axis-value inference, broken-axis segment-coverage diagnostics, automatic quality review, persisted chart review action queues, and in-workbench chart-review state editing; the next gaps are richer batch review workflows, more complex broken-axis layouts, and higher-confidence dense chart extraction.
