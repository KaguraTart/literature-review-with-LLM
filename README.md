...
- `Model`: model name used by the selected provider. Pick from the provider-specific model dropdown first. Aggregators such as OpenRouter, LiteLLM, and Cline API also show a `Model vendor` dropdown, so users can choose Anthropic, Google Gemini, OpenAI, DeepSeek, MiniMax, and then pick a concrete model. Click `Load model list` to show built-in recommendations first and then append the provider's online model list when the profile supports it and an API key is available. Azure OpenAI deployments and private gateways may still require your own deployment/model name through `Custom/private model`.
