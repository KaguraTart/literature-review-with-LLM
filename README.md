...
- Harden PDF page extraction beyond the current Zotero page text, local path/base64 bridge, bounded OCR fallback path with sparse-page selection, candidate-report extraction-quality rows, structured extraction diagnostics, per-page OCR confidence signals, calibrated OCR confidence risk summaries, unavailable-byte/bridge-request diagnostics, and indexed-text fallback follow-up actions, especially full-document scanned OCR beyond the bounded OCR window, no-bridge raw-byte extraction, and automatic repair.
