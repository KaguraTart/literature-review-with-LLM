...
- Continue hardening chart digitization beyond the current prompt-level visual OCR contract, editable local OCR metadata, JSON/CSV table sidecar export, reviewable chart-data drafts, model-estimated pixel/coordinate drafts, axis-calibration anchor export, calibration-quality diagnostics, automatic quality review, and stateful chart review action queues; the next gaps are denser point extraction, more precise automatic digitization, and persisted review-state editing inside the workbench.
