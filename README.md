...
- Harden PDF page extraction beyond the current Zotero page text, local path/base64 bridge, bounded OCR fallback path with sparse-page selection, automatic high-DPI and bounded PSM repair retries for empty or failed OCR pages, explicit full-document OCR opt-in under reviewed page caps, candidate-report extraction-quality rows, structured extraction diagnostics, per-page OCR confidence/repair signals, calibrated OCR confidence risk summaries, unavailable-byte/bridge-request diagnostics, and indexed-text fallback follow-up actions, especially no-bridge raw-byte extraction, image preprocessing/deskew before OCR, and stronger scanned-document layout recovery.
