...
- **Candidate-paper review queue**: arXiv / Crossref / Semantic Scholar / Unpaywall results are deduplicated, ranked with explainable priority signals, expanded through configurable bounded Semantic Scholar references/citations policies, reviewed with saved manual notes, optionally updated from high-confidence recommendations, exported as a Markdown review report, saved as JSONL, and imported only after manual review.
...
- Candidate-paper search now has explainable ranking, duplicate reconciliation, configurable bounded citation-network expansion, saved manual review notes, high-confidence recommendation application, and a Markdown candidate-review report, but richer review workflows still need more work.
